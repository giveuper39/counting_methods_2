...
def run_all(f, grad_f, hess_f, x0, x_prec, learning_rate=0.01):
    x_gd, _, iter_gd = gradient_descent(f, grad_f, x0, learning_rate=learning_rate)
    x_sd, _, iter_sd = fast_descent(f, grad_f, x0)
    x_hb, _, iter_hb = heavy_ball(f, grad_f, x0, learning_rate=learning_rate)
...
def gradient_descent(f, grad_f, x0, learning_rate=0.01, max_iter=100000, tol=1e-6):
    x = x0.copy()
    history = [x0]
...
def fast_descent(f, grad_f, x0, max_iter=100000, tol=1e-6):
    x = x0.copy()
    history = [x0]
    for i in range(max_iter):
        grad = grad_f(x)
...
def heavy_ball(f, grad_f, x0, learning_rate=0.01, momentum=0.9, max_iter=100000, tol=1e-6):
    x = x0.copy()
    history = [x0]
    v = np.zeros_like(x0)
    for i in range(max_iter):
...
def newton(f, grad_f, hess_f, x0, max_iter=10000, tol=1e-6):
    x = x0.copy()
    history = [x0]
    for i in range(max_iter):
        grad = grad_f(x)
